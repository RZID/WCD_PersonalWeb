...
const ContactPage = () => {
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    console.log(e);
    window.open(
      `mailto:${import.meta.env.VITE_APP_OWNER_EMAIL_ADDRESS}?subject=${
        formData.subject
      }&body=Hello, my name is ${formData.name}. ${formData.subject}`,
      "_blank"
    );
  };
              ...
              {socialLinks.map((social, i) => (
                <div className={classNames("flex")}>
                  <motion.p
                    ...
                    onClick={() =>
                      social.link && window.open(social.link, "_blank")
                    }
                  >
                    {social.name}
                  </motion.p>
        ...
        >
          ...
          >
            ...
            <motion.div whileHover={{ scale: 1.05 }}>
              <Button
                variant="dark"
                className={classNames("!px-16", "font-medium")}
              >
                Submit
              </Button>
            </motion.div>
